...
AMyProject3Character::AMyProject3Character()
{
	...
	TriggerVolume->InitSphereRadius(4000.0f);

	TriggerVolume->OnComponentBeginOverlap.AddDynamic(this, &AMyProject3Character::OnOverlapBegin);*/
...
void AMyProject3Character::SetupPlayerInputComponent(class UInputComponent* PlayerInputComponent)
{
	...
	PlayerInputComponent->BindAxis("Chardge", this, &AMyProject3Character::Chardge);
...
/*void LockCamera(FVector Value)
{
	
}*/

void AMyProject3Character::Chardge(float Value) {
	if (Stamina > 30) {
		if ((Value == 1) && (IsAnimating == 1)) {
			FVector FCharacterVector;
			FCharacterVector = AMyProject3Character::GetActorForwardVector();
			AMyProject3Character::SetActorLocation(AMyProject3Character::K2_GetActorLocation() + (FCharacterVector * 400), true, nullptr, ETeleportType::None);
			FastChardge = true;
			Stamina -= 10;
		}
	}
